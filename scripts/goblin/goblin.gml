...
{
	//colisão horizontal
if place_meeting(x+hspd, y, obj_colliders)
{
	while (!place_meeting(x+sign(hspd),y,obj_colliders))
	{
		x = x+sign(hspd);
	}
	hspd = 0;
...
x = x +hspd;
//colisão vertical
if place_meeting(x, y+vspd, obj_colliders)
{
	while (!place_meeting(x,y+sign(vspd),obj_colliders))
	{
		y = y+sign(vspd);
	}
	vspd = 0;
